/**
 * copy from https://blog.csdn.net/qq_24451605/article/details/50096487
 */
#include <iostream>
#include <cstring>
#include <cstdio>
...
#define DEBUG
    ...
    for (int i = 0; i < VT.size(); i++)
    {
        ...
        for (int j = 0; j < VT.size(); j++)
        {
            string end = j == VT.size() - 1 ? "}," : ", ";
            char r = relation[VT[i]][VT[j]];
            printf("%s%s", r == '<' ? "-1" : r == '>' ? "1" : r == '=' ? "0" : "null", end.c_str());
        }
        puts("");
    }    
}
